...
module fp_compare(clk, pc, a, b, start, cmp_type, y, fflags);
   parameter W = 32;
   parameter D = 4;
   ...
   output logic        y;
   output logic [4:0]  fflags;   // IEEE flags {V,Z,O,U,I}; a compare only raises V
   ...
   assign y = r_d[D-1];
   assign fflags = {r_v[D-1], 4'b0};   // only V (invalid) can be raised
   generate
      assign w_d[0] = t_y;
...
`endif
   ...
   // IEEE: comparisons are unordered (result false) when either operand is NaN.
   wire w_unordered = a_is_nan | b_is_nan;
   // signaling NaN = NaN with the quiet bit (frac MSB) clear
   wire w_a_snan = a_is_nan & ~a[F-1];
   wire w_b_snan = b_is_nan & ~b[F-1];
   wire w_any_snan = w_a_snan | w_b_snan;
   always_comb
     begin
	t_y = 1'b0;
	if(!w_unordered)
   ...
   // Invalid (V): LT/LE are signaling predicates -> raise on any NaN; EQ is the
   // quiet predicate -> raise only on a signaling NaN.
   always_comb
     begin
	t_invalid = 1'b0;
	case(cmp_type)
